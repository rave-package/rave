			     ...
			     const ravesurf::Cylinder& raveCyl)
{
  ...
  if (fabs(rho)*R < sraightLineCutoff &&
      fabs(rho)*startingPos.perp()  < sraightLineCutoff) {
    // switch to straight line case
    StraightLineCylinderCrossing slc( raveCyl.toLocal(startingPos),
				      raveCyl.toLocal(startingDir), propDir);
    std::pair<bool,double> pl = slc.pathLength( raveCyl);
    if (pl.first) {
      theSolExists = true;
      theS = pl.second;
      thePos = raveCyl.toGlobal(slc.position(theS));
      theDir = startingDir;
    }
    else theSolExists = false;
    return; // all needed data members have been set
